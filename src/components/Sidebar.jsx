...
import { Home, User, MessageCircle, PlusCircle, Search } from "lucide-react";
...
export default function Sidebar() {
  ...
  const navItems = [
    ...
    { name: "Search", icon: <Search size={26} />, path: "/search" },
          ...
          return (
            <Link
              ...
              className={`flex items-center gap-4 p-3.5 rounded-xl transition-all group ${
                isActive ? "font-bold text-foreground" : "text-muted-foreground hover:bg-muted hover:text-foreground"
              }`}
            >
